...
class FileOperationsNotifier extends StateNotifier<AsyncValue<void>> {
  ...
  FileOperationsNotifier(this._apiClient) : super(const AsyncData(null));
  ...
  Future<bool> uploadFile(
    ...
    final systemInfo = ref.read(systemInfoProvider).valueOrNull;
...
final fileOpsProvider =
  ...
  return FileOperationsNotifier(client);
});
...
final fileListProvider =
  ...
  Future<void> fetch() async {
    try {
      ...
      if (!controller.isClosed) {
        timer = Timer(const Duration(seconds: 10), fetch);
      }
    }
  }
