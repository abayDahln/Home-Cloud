...
func getDiskLabel(p disk.PartitionStat) string {
	...
	switch runtime.GOOS {
	case "windows":
		label = getWindowsVolumeLabel(p.Mountpoint)
	case "linux":
		label = getLinuxDiskLabel(p)
	case "darwin":
		label = getMacDiskLabel(p)
	default:
		label = ""
	}
...
func getMacDiskLabel(p disk.PartitionStat) string {
...
func authMiddleware(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if (auth == "" || !strings.HasPrefix(auth, "Bearer ") || strings.TrimPrefix(auth, "Bearer ") != authToken) && token != authToken {
			log.Printf("Unauthorized [%s]: Path=%s Remote=%s", r.Method, r.URL.Path, r.RemoteAddr)
...
func settingsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		if s.StorageQuotaGB < 50 || s.StorageQuotaGB > 200 {
			http.Error(w, "Quota must be between 50 and 200 GB", http.StatusBadRequest)
			return
		}
