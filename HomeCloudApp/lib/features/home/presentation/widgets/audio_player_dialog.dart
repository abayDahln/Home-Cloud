...
class _AudioPlayerDialogState extends State<AudioPlayerDialog> {
  ...
  Future<void> _initializePlayer() async {
    try {
      ...
      // Add timeout handling for mobile audio playback
      await _audioPlayer.play(UrlSource(finalUrl)).timeout(
        const Duration(seconds: 60),
        onTimeout: () {
          throw Exception(
              'Connection timeout - Please check your network and server connection');
        },
      );
  ...
  String _getReadableError(String error) {
    if (error.contains('timeout') || error.contains('TimeoutException')) {
      return 'Connection timeout. Please check your network and server connection.';
    }
