...
final streamApiClientProvider = Provider<ApiClient>((ref) {
  ...
  String serverUrl = authState.serverUrl ?? 'http://localhost:8090';

  // UI-side migration for the stream client specifically
  if (serverUrl.contains(':8080') &&
      (serverUrl.contains('localhost') || serverUrl.contains('127.0.0.1'))) {
    serverUrl = serverUrl.replaceFirst(':8080', ':8090');
  }
...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Future<void> _requestPermissions() async {
    if (Platform.isAndroid) {
      ...
      // Required for reliable exact alarms on Android 12+
      ...
      // The user wants this prompt to appear immediately
      if (!await Permission.ignoreBatteryOptimizations.isGranted) {
        await Permission.ignoreBatteryOptimizations.request();
      }

      // System Alert Window for persistent UI (optional but good for foreground services sometimes)
      ...
      children: [
        RefreshIndicator(
          onRefresh: () async {
            ...
            ref.invalidate(liveSystemInfoProvider);
    ...
    try {
      if (['mp4', 'mov', 'avi', 'mkv', 'webm'].contains(ext)) {
        ...
        if (!(Platform.isWindows || Platform.isLinux || Platform.isMacOS)) {
          ...
          try {
            await preInitializedController
                .initialize()
                .timeout(const Duration(seconds: 30));
          } catch (e) {
            debugPrint('⚠️ Video pre-init failed: $e');
            // We'll let the dialog handle the error state if init fails here
          }
        }
        ...
        if (context.mounted) {
          ...
          showDialog(
            ...
            builder: (context) => VideoPlayerDialog(
              ...
              allFiles: allFiles,
              initialFile: file,
              preInitializedController: preInitializedController,
            ),
          );
        } else if (preInitializedController != null) {
          preInitializedController.dispose();
        }
      ...
      items: isSelectionMode
          ...
          : [
              ...
              _buildPopupItem(Icons.info_outline_rounded, 'Details',
                  () => _showFileDetails(context)),
              _buildPopupItem(Icons.copy_rounded, 'Copy full path', () async {
                final path = file.fullPath ?? file.path;
    ...
    _showStyledDialog(
      ...
      child: Column(
        ...
        children: [
          ...
          _DetailLine(label: 'Full Path', value: file.fullPath ?? file.path),
        ],
      ),
      onConfirm: () {},
    );
