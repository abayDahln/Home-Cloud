...
class BackupService {
  ...
  Future<void> _handleFileChange(String filePath, BackupFolder folder,
    ...
    if (_processingFiles.contains(filePath)) {
      debugPrint(
          '⏳ [BackupService] Ignoring concurrent event for: ${p.basename(filePath)}');
      return;
    }
    ...
    try {
      ...
      // This fixes the 0-byte file issue on Windows where file is detected before content is written
      ...
      try {
        int lastSize = await file.length();

        // If file is empty, wait and check again
        if (lastSize == 0) {
          await Future.delayed(const Duration(seconds: 2));
          if (!file.existsSync()) return;
          lastSize = await file.length();
        }
        ...
        if (currentSize != lastSize) {
          debugPrint(
              '⏳ [BackupService] File is still being written: ${p.basename(filePath)}');
          // File is changing, schedule a retry.
          // IMPORTANT: Remove from processing set so the retry can actually run!
          _processingFiles.remove(filePath);
          await Future.delayed(const Duration(seconds: 2));
          return _handleFileChange(filePath, folder,
              cancelToken: cancelToken, bypassHistory: bypassHistory);
        }

        if (currentSize == 0) {
          debugPrint(
              '⚠️ [BackupService] Skipping empty file: ${p.basename(filePath)}');
          return;
        }
      } catch (e) {
        debugPrint('⚠️ [BackupService] Error checking file stability: $e');
        return;
      }
      ...
      // Always clear the processing flag when done (or if error occurred)
      // Note: If we recursed (retry), we handled the removal there.
      // But for normal flow or error, we remove it here.
      // Ideally, the retry logic should just be a loop or await, but recursion is used.
      // If we recursed, we already removed it *before* the recursion call above.
      // So here we only remove if it's still in the set (which is fine).
      _processingFiles.remove(filePath);
    }
  }
