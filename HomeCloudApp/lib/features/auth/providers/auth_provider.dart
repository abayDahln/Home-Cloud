...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<String?> login(String serverUrl, String password,
    ...
    try {
      ...
      // 3. Auto-add port 8090 ONLY if it's not HTTPS and not a Cloudflare Tunnel
      ...
      if (!uri.hasPort &&
        ...
        formattedUrl = '$formattedUrl:8090';
        print('🔧 [AuthNotifier] No port detected, defaulted to :8090');
      }

      // 4. Migration: If URL is exactly localhost/127.0.0.1 on port 8080, move to 8090
      if (formattedUrl.contains(':8080') &&
          (formattedUrl.contains('localhost') ||
              formattedUrl.contains('127.0.0.1') ||
              formattedUrl.contains('10.0.2.2'))) {
        formattedUrl = formattedUrl.replaceFirst(':8080', ':8090');
        print('🔧 [AuthNotifier] Port migration: 8080 -> 8090');
      }
      ...
      if (e.type == DioExceptionType.connectionTimeout ||
        ...
        return 'Server timed out. Please check your connection and ensure the server is running.';
      } else if (e.type == DioExceptionType.connectionError) {
        return 'Could not reach server. Please check the URL and your internet connection.';
      } else if (e.type == DioExceptionType.badResponse) {
        final status = e.response?.statusCode;
        if (status == 401) {
          return 'Invalid password. Please try again.';
        } else if (status == 404) {
          return 'Server endpoint not found. Please check your URL.';
        } else if (status == 500) {
          return 'Server error. Please check the server logs.';
        }
        return 'Server returned error: $status';
      }
      return 'Connection error. Please check your server URL.';
    } catch (e) {
