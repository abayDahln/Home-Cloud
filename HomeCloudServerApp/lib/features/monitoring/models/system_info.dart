class SystemInfo {
  ...
  factory SystemInfo.empty() {
    return SystemInfo(
      ...
      sys: SysInfo.empty(),
      cpu: [CpuInfo.empty()],
      cpuProcess: CpuProcess.empty(),
      process: ProcessInfo.empty(),
      memory: MemoryInfo.empty(),
      disks: [],
      network: NetworkSpeed.empty(),
    );
  }
...
class OsInfo {
  ...
  factory OsInfo.empty() {
    return OsInfo(goVersion: '', os: '', arch: '', cpuCount: '0');
  }
}
...
class SysInfo {
  ...
  factory SysInfo.empty() {
    return SysInfo(hostname: '', os: '', platform: '', kernelArch: '');
  }
}
...
class CpuProcess {
  ...
  factory CpuProcess.empty() {
    return CpuProcess(usage: [0]);
  }
...
class ProcessInfo {
  ...
  factory ProcessInfo.empty() {
    return ProcessInfo(count: 0, threads: 0);
  }
}
...
class MemoryInfo {
  ...
  factory MemoryInfo.empty() {
    return MemoryInfo(total: 100, used: 0, available: 0, free: 0);
  }
...
class NetworkSpeed {
  ...
  factory NetworkSpeed.empty() {
    return NetworkSpeed(
        downloadMBps: 0, uploadMBps: 0, downloadMbps: 0, uploadMbps: 0);
  }
}
